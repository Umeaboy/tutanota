...
/**
 * used to transform whatever is typed/pasted into the expiration date field into something
 * standardized while it's being typed, while inferring as much as possible.
 *
 * the only assumption about the year we make here is that it's within the 2000-2099 range.
 *
...
function inferNormalizedExpirationDate(value: string, oldDate: string): string {
	...
	if ((ret.length === 2 && rest.length > 0) || hadSlash) {
		// if there is more input or the user added a slash at
		// the end of the month, we need a slash
		ret += separator
	}
...
export class CreditCardViewModel implements CCViewModel {
